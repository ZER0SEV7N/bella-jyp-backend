...
export function ApiSwaggerLogin() {
    return applyDecorators(
        ApiOperation({
            ...
            description: 'Valida credenciales. Devuelve un JWT en JSON y Refresh Token en Cookie.',
        }),
        ApiBody({
            schema: {
                type: 'object',
        ...
        ApiResponse({
            status: 200,
            description: 'Login exitoso. Devuelve Access Token en JSON y Refresh Token en Cookie.',
        }),
        ApiResponse({
            status: 401,
...
export function ApiSwaggerRefresh() {
    return applyDecorators(
        ApiOperation({
            ...
            description: 'Valida el Refresh Token en Cookie y devuelve un nuevo Access Token en JSON.',
        }),
        ApiResponse({
            status: 200,
            description: 'Refresh Token válido. Devuelve un nuevo Access Token en JSON.',
        }),
        ApiResponse({
            status: 401,
            description: 'Refresh Token inválido o expirado. Devuelve un mensaje de error.',
        })
    );
}
...
export function ApiSwaggerProvisionar() {
    return applyDecorators(
        ...
        ApiOperation({
            ...
            description: 'Crea un nuevo usuario interno. Solo accesible para roles ADMIN o RRHH.',
        }),
        ApiBody({
            schema: {
                type: 'object',
        ...
        ApiResponse({
            status: 201,
            description: 'Usuario creado exitosamente. Devuelve el ID y rol del nuevo usuario.',
        }),
        ApiResponse({
            status: 400,
        ...
        ApiResponse({
            status: 403,
            description: 'Acceso denegado. El usuario no tiene permisos para provisionar usuarios.',
        })
    );
}
/**
...
export function ApiSwaggerRecuperarPassword() {
    return applyDecorators(
        ApiOperation({
            ...
            description: 'Solicita la recuperación de contraseña enviando un correo con un enlace de restablecimiento.',
        }),
        ApiBody({
            schema: {
                type: 'object',
