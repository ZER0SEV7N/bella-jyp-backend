//src/modules/auth/use-cases/recuperacionPassword.useCases.ts
...
export class RecuperacionPasswordUseCases {
    ...
    async solicitar(dto: SolicitudRecuperacionDTO) {
        ...
        if (!usuario) return { message: 'Si el documento es válido, se enviarán las instrucciones.' };
    ...
    async restablecer(dto: RestablecerPasswordDTO) {
        ...
        if (!tokenRecord || tokenRecord.usado || tokenRecord.proposito !== 'RESET_PASSWORD') throw this.tokenInvalido();
    
        //Validar que el token no haya expirado
        if(new Date() > tokenRecord.expira_en) throw new BadRequestException({
    ...
    private dispararWebhookN8n(email: string, token: string){
        ...
        const signature = crypto
            .createHmac('sha256', process.env.N8N_WEBHOOK_SECRET || 'SECRET_TEMPORAL_DEV')
            .update(payload)
            .digest('hex');
        ...
        if(!webHookUrl) throw new Error('N8N_WEBHOOK_URL_RESET_PASSWORD no está configurado en las variables de entorno.');
