...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
    constructor(configService: ConfigService) {
        super({
            ...
            secretOrKet: configService.get<string>('JWT_SECRET'),
        });
    }
        
    //Metodo para validar el token, se ejecuta cada vez que se hace una petición con un token válido
    async validate(payload: any) {
        if(!payload.sub || !payload.email) throw new UnauthorizedException('Token inválido');
        
        return { id: payload.sub, rol:payload.rol, nro_documeto: payload.doc, empleado_id: payload.empId };
    }
}