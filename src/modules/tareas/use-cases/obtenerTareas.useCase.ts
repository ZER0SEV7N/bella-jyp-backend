...
export class ObtenerTareasUseCase {
    ...
    async execute(query: ObtenerTareasQueryDto, userRequesting: {id: string, rol: string}) {
        ...
        const [total, tareas] = await this.prisma.$transaction([
            ...
            this.prisma.tareas_asistente.findMany({
                ...
                usuarios_tareas_asistente_asignado_aTousuarios: { select: { email: true, rol: true } },
                usuarios_tareas_asistente_asignado_porTousuarios: { select: { email: true, rol: true } },
