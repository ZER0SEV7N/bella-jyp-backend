...
export class AgregarTipoAfpUseCase {
  ...
  async execute(dto: CrearTipoAfpDto) {
    try {
      ...
      if (!regimen || !regimen.nombre.includes('AFP'))
        throw new BadRequestException({
          title: 'Régimen Inválido',
          detail:
            'El régimen de pensión seleccionado no existe o no corresponde a un sistema AFP.',
        });

      //Validar que no se duplique el nombre del tipo de AFP
      const afpExistente = await this.prisma.tipo_afp.findFirst({
