  ...
  export class DatoFinancieroController {
     ...
     *    - cuenta_bancaria: string - Número de cuenta bancaria del empleado (opcional)
     *    - cci: string - Código de cuenta interbancario del empleado (opcional)
     *    - nro_cuenta_cts: string - Número de cuenta CTS del empleado (opcional)
     *    - sueldo_basico: number - Sueldo básico del empleado (opcional)
     *    - cuspp: string - CUSPP del empleado (opcional)
     *    - tipo_comision: TipoComisionAfpEnum - Tipo de comisión de AFP del empleado (opcional)
     * }
     */
    @Post()
    @Roles('ADMIN', 'CONTADOR', 'RRHH')
