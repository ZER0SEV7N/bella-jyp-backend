...
export function ApiSwaggerComisionCrear() {
  return applyDecorators(
      ...
      schema: {
        ...
        properties: {
          tipo_afp_id: {
            type: 'string',
            format: 'uuid',
            description: 'ID de la AFP a la que aplica',
          },
          anterior_comision: {
            type: 'object',
            description:
              'Opcional. Datos de la comisión vigente para cerrarla.',
            properties: {
          ...
          nueva_comision: {
            ...
            properties: {
              ...
              aporte_obligatorio: { type: 'number', example: 10.0 },
    ...
    ApiResponse({
      status: 404,
      description: 'No se encontró la AFP o la comisión anterior especificada.',
    }),
    ApiResponse({
      status: 500,
      description:
...
export function ApiSwaggerComisionListar() {
  return applyDecorators(
    ...
    ApiQuery({
      ...
      type: Boolean,
      example: true,
      description: 'Trae solo las tasas actuales (sin fecha de fin)',
    }),
    ApiResponse({
      status: 200,
      description: 'Listado de comisiones obtenido exitosamente.',
    }),
    ApiResponse({
      status: 401,
      description: 'No autorizado. Token JWT inválido o ausente.',
...
export function ApiSwaggerTipoAfpCrear() {
  return applyDecorators(
      ...
      schema: {
        ...
        properties: {
          ...
          id_regimen: {
            type: 'string',
            format: 'uuid',
            description: 'ID del Régimen Privado',
          },
        },
      },
    }),
    ApiResponse({
      status: 201,
      description: 'Tipo de AFP registrado exitosamente.',
    }),
    ApiResponse({
      status: 400,
      description: 'Error de validación o datos duplicados.',
...
export function ApiSwaggerTipoAfpListar() {
  return applyDecorators(
    ...
    ApiResponse({
      status: 200,
      description: 'Listado de tipos de AFP obtenido exitosamente.',
    }),
    ApiResponse({
      status: 401,
      description: 'No autorizado. Token JWT inválido o ausente.',
