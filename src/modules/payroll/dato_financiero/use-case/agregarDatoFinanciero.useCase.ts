...
export class agregarDatosFinancieroUseCase {
  ...
  async execute(dto: crearDatoFinancieroDto) {
    ...
    if (!empleado || empleado.activo !== false || empleado.deleted_at !== null) throw new NotFoundException('empleado no valido');
    
    //validar la existencia del banco
    if (!banco) throw new NotFoundException('banco no encontrado');
    
    if (!regimen) throw new NotFoundException('regimen no valido');
    ...
    try {
      ...
      throw new InternalServerErrorException('error al ingresar datos financieros de empleado', error instanceof Error ? error.message : String(error));
    }
  }
}
