...
export class obtenerDatoFinancieroUseCase {
  ...
  async execute(idEmpleado: string) {
    ...
    //validar empleado
    if (!empleado || empleado.deleted_at !== null || empleado.activo !== true) throw new NotFoundException('empleado no encontrado o inactivo');
    
    try {
      //datos financiero de empleado
      const datos_financiero = await this.prisma.dato_financiero.findUnique({
        where: {empleado_id: empleado.id, deleted_at: null},
        select: {
          id: true,
