...
export class RenovarContratoUseCase {
  ...
  async execute(idContrato: string) {
    ...
    //validar el empleado
    if (!contratoValidado.empleados || contratoValidado.empleados.activo !== true) throw new NotFoundException('Empleado no encontrado o no cumple con lo requisitos');
    ...
    try {
      ...
      throw new InternalServerErrorException('error al renova contrato', error instanceof Error ? error.message : String(error));
    }
  }
}
