...
export class EditarContratoUseCase {
  ...
  async execute(dto: editarContratDto, idContrato: string) {
    ...
    if (!empeladoValidado) throw new NotFoundException('empleado no existente o inactivo');
    //validacion de contrato
    if (!contrato || contrato.deleted_at !== null) throw new NotFoundException('contrato no existente o inactivo');
    ...
    try {
      ...
      throw new InternalServerErrorException('ocurrio un error al editar datos del contrato', error instanceof Error ? error.message : String(error));
    }
  }
}
