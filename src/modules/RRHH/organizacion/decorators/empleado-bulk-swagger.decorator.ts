...
export function ApiSwaggerEmpleadosBulkController() {
  return applyDecorators(
    ApiTags('Modulo RRHH - Carga Masiva CSVCarga Masiva CSV'),
...
export function ApiSwaggerUploadBulk() {
  return applyDecorators(
    ApiOperation({
      ...
      description:
        'Permite subir un archivo CSV para procesar la carga masiva de empleados. Requiere rol ADMIN o RRHH.',
    }),
      ...
      schema: {
        ...
        properties: {
          file: {
            ...
            format: 'binary',
            description:
              'Archivo CSV que contiene los datos de los empleados a cargar.',
          },
        },
      },
    }),
    ApiResponse({
      status: 202,
      description:
        'Archivo recibido y procesamiento iniciado. Retorna un jobId para consultar el estado del procesamiento.',
    }),
    ApiResponse({
      status: 400,
      description:
        'Solicitud inválida. Devuelve un mensaje de error si el archivo no es válido o no se encuentra.',
    }),
    ApiResponse({
      status: 401,
...
export function ApiSwaggerGetBulkStatus() {
  return applyDecorators(
    ApiOperation({
      ...
      description:
        'Permite consultar el estado del procesamiento de un jobId generado al subir un archivo CSV para carga masiva de empleados. Requiere rol ADMIN o RRHH.',
    }),
    ApiParam({
      name: 'jobId',
    ...
    ApiResponse({
      status: 200,
      description:
        'Retorna el estado actual del job, incluyendo total de registros, procesados y fallidos.',
    }),
    ApiResponse({
      status: 400,
      description:
        'Solicitud inválida. Devuelve un mensaje de error si el jobId no es válido o no se encuentra.',
    }),
    ApiResponse({
      status: 401,
      description: 'No autorizado. El usuario no tiene un token válido.',
