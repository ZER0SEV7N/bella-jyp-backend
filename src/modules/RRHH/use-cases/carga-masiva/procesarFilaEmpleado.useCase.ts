...
export class ProcesarFilaEmpleadoUseCase {
  ...
  async execute(fila: CargaMasivaFilaDTO, jobId: string): Promise<void> {
    ...
    const tipoDoc = await this.prisma.tipo_documento.findFirst({ where: { tipo_documento: fila.tipo_documento } });
    ...
    if (!nombreValidado || !apellidoValidado) throw new Error(`Imposible registrar DNI ${fila.nro_documento}: Nombres ausentes y RENIEC inoperativo.`);
    ...
    if (!estadoBase) throw new Error('No existe el estado de empleado base en el catálogo.');
