...
describe('CrearJornadaUseCase - Pruebas Unitarias Exhaustivas', () => {
  ...
  // Mockear las funciones del servicio Prisma adaptadas a los helpers centralizados
  ...
  // Definir un horario semanal válido para las pruebas (L-V 8h diarias = 40h)
  ...
  describe('Casos de Éxito (Happy Path)', () => {
    it('Debe registrar exitosamente una jornada descontando refrigerio y asociando áreas', async () => {
      // Arrange: Simular que no existe una jornada con el mismo nombre y que las 2 áreas existen activas
      mockPrismaService.jornada.findFirst.mockResolvedValue(null);
      mockPrismaService.area.count.mockResolvedValue(2);
      ...
      expect(prisma.jornada.findFirst).toHaveBeenCalledWith({
        where: { nombre: { equals: 'Jornada Administrativa 40h', mode: 'insensitive' }, deleted_at: null },
        select: { id: true },
      });
      ...
      // Verificar que se asociaron las áreas mediante la tabla pivote
  ...
  // =========================================================================
  // 2. Validaciones de Negocio y Excepciones
  // =========================================================================
  describe('Validaciones de Negocio y Excepciones', () => {
    it('Debe lanzar BadRequestException si el nombre ya existe', async () => {
      ...
      expect(prisma.area.count).not.toHaveBeenCalled();
    });

    it('Debe lanzar NotFoundException si alguna área no existe o está inactiva', async () => {
      // Arrange: Simular que no existe una jornada con el mismo nombre y que solo 1 de las 2 áreas está activa
      mockPrismaService.jornada.findFirst.mockResolvedValue(null);
      mockPrismaService.area.count.mockResolvedValue(1); // Esperaba 2
    ...
    it('Debe lanzar BadRequestException si el turno es ROTATIVO pero no incluye patron_rotacion', async () => {
      ...
      // Simular que no existe duplicado y que las áreas existen
      mockPrismaService.jornada.findFirst.mockResolvedValue(null);
      mockPrismaService.area.count.mockResolvedValue(2);
    ...
    it('Debe lanzar BadRequestException si excede el límite legal de 48 horas semanales', async () => {
      // Arrange: Preparar un horario semanal que exceda las 48 horas (5 días x 10h = 50 horas semanales)
      ...
      // Simular que no existe colisión de nombre y que las áreas existen
      mockPrismaService.jornada.findFirst.mockResolvedValue(null);
      mockPrismaService.area.count.mockResolvedValue(2);

      // Act & Assert: Ejecutar el caso de uso y esperar que lance BadRequestException por exceder 48 horas
    ...
    it('Debe lanzar BadRequestException si es TIEMPO_PARCIAL pero supera o iguala las 30 horas', async () => {
      // Arrange: Simular que no hay colisión y que las áreas existen, pero configurado con 40h y duración TIEMPO_PARCIAL
      mockPrismaService.jornada.findFirst.mockResolvedValue(null);
      mockPrismaService.area.count.mockResolvedValue(2);

      // Act & Assert: Ejecutar el caso de uso y esperar que lance BadRequestException por exceder 30 horas para tiempo parcial
