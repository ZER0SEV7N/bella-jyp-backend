...
import { ActiveCargoUseCase } from '@/modules/RRHH/organizacion/use-cases/cargos/activeCargo.useCase';
...
describe('ActiveCargoUseCase', () => {
  ...
  let mockPrisma = {cargo: { update: jest.fn() }};
  ...
  it('debe lanzar BadRequestException si prisma falla al actualizar (ej: id no existe)', async () => {
    ...
    mockPrisma.cargo.update.mockRejectedValueOnce(new Error('Record to update not found'));
  ...
  it('debe lanzar ZodError si el id no es un string (no lo envuelve en BadRequestException)', async () => {
    ...
    await expect(useCase.execute(123 as unknown as string)).rejects.toThrow(ZodError);
