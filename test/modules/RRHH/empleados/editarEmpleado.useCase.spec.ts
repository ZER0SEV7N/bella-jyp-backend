...
describe('EditarEmpleadoUseCase', () => {
    ...
    it('Debería actualizar el empleado correctamente sin cambio de documento', async () => {
        ...
        mockPrisma.empleados.findUnique.mockResolvedValue({ id: 'emp-123', nro_documento: '7011', deleted_at: null });
        mockPrisma.empleados.update.mockResolvedValue({ id: 'emp-123', nombre: 'Juan Editado' });
    ...
    it('Debería actualizar el documento si el nuevo no colisiona', async () => {
        ...
        mockPrisma.empleados.findUnique.mockResolvedValueOnce({ id: 'emp-123', nro_documento: '7011', deleted_at: null });
    ...
    it('Debería lanzar BadRequestException si el nuevo documento ya pertenece a otro', async () => {
        ...
        await expect(useCase.execute('emp-123', payload as any)).rejects.toMatchObject({
    ...
    it('Debería lanzar NotFoundException si el empleado no existe o está eliminado', async () => {
        ...
        mockPrisma.empleados.findUnique.mockResolvedValue({ id: 'emp-123', deleted_at: new Date() });
