...
describe('CrearEmpleadoUseCase', () => {
    ...
    it('Debería crear un empleado con datos completos sin llamar a RENIEC', async () => {
        ...
        const payload = { nro_documento: '70112233', nombre: 'Juan', apellido: 'Perez', asig_familiar: false };
    ...
    it('Debería llamar a RENIEC si faltan nombres y es un DNI (8 dígitos)', async () => {
        ...
        mockReniec.consultarDni.mockResolvedValue({nombre: 'Carlos', apellido_paterno: 'Lopez', apellido_materno: 'Gomez'});
        ...
        expect(mockPrisma.empleados.create).toHaveBeenCalledWith(
            expect.objectContaining({data: expect.objectContaining({ nombre: 'Carlos', apellido: 'Lopez Gomez' })})
        );
    });
