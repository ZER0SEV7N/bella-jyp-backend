...
describe('ActualizarAreaUseCase', () => {
    ...
    it('Debería actualizar el área exitosamente', async () => {
        ...
        mockPrisma.area.findUnique.mockResolvedValue({ id: idArea, deleted_at: null });
        ...
        expect(mockPrisma.area.update).toHaveBeenCalledWith({ where: { id: idArea }, 
    ...
    it('Debería lanzar NotFoundException si el área existe pero tiene Soft Delete', async () => {
        ...
        mockPrisma.area.findUnique.mockResolvedValue({ id: 'uuid-123', deleted_at: new Date() });
    ...
    it('Debería lanzar InternalServerErrorException si la base de datos falla en el update', async () => {
        //Arrange
        mockPrisma.area.findUnique.mockResolvedValue({ id: 'uuid-123', deleted_at: null });
