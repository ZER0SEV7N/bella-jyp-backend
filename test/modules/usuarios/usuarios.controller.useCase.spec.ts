...
describe('UsuariosController', () => {
    ...
    it('obtenerMiPerfil(): Debería extraer el ID del request y retornar el perfil', async () => {
        ...
        expect(mockObtenerMiPerfilUC.obtenerPerfil).toHaveBeenCalledWith('user-123');
    });

    it('obtenerMiPerfil(): Debería lanzar un error si ObtenerMiPerfilUseCase falla', async () => {
        //Arrange
        const mockReq = { user: { id: 'user-123' } } as any;
        mockObtenerMiPerfilUC.obtenerPerfil.mockRejectedValue(new Error('Error inesperado'));

        //Act & Assert
        await expect(controller.obtenerMiPerfil(mockReq)).rejects.toThrow('Error inesperado');
        expect(mockObtenerMiPerfilUC.obtenerPerfil).toHaveBeenCalledWith('user-123');
    });
